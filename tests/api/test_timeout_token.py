...
class TimeoutTokenCase(unittest.TestCase):
    ...
    def test_json_token_notstream(self):
        result = '{"msg":"","timestamp":1659539571639,"cid":"a6fe6a6ec8bc",' \
    ...
    def test_json_compact_token_notstream(self):
        result = '{"msg":"","status":0,"timestamp":1659604839206,' \
    ...
    def test_json_simple_token_notstream(self):
        ...
        self.assertIsNotNone(response)
        self.assertEqual([16, 44], [json.loads(event)["count"] for event in
                                    response.split('\r\n')])

    def test_json_simple_token_stream(self):
        result = iter(
            [b'    ', b'       {"eventdate":1659615360000,"count":118}',
             b'     ',
        ...
        self.assertIsNotNone(response)
        self.assertEqual([118, 14],
                         [json.loads(event)["count"] if event.strip() else ''
                          for event in response])

    def test_json_simple_compact_token_notstream(self):
        result = '{"m":{"eventdate":{"type":"timestamp","index":0},"count":' \
        ...
        self.assertEqual([10, 40],
                         [json.loads(event)["d"][1] for event in
    ...
    def test_csv_default_token_notstream(self):
        ...
        response = self._query_no_stream(response, result,
                                         keepAliveToken=keepalive_token)

        self.assertIsNotNone(response)
        self.assertEqual([('2022-08-04 10:47:00.000', '59'),
                          ('2022-08-04 10:48:00.000', '4')],
                         [(event.split(',')[0], event.split(',')[1]) for event
                          in response.split('\n')[1:]])
    ...
    def test_csv_empty_event_token_notstream(self):
        ...
        self.assertIsNotNone(response)
        self.assertEqual([('2022-08-04 10:52:00.000', '40'),
                          ('2022-08-04 10:53:00.000', '16')],
                         [(event.split(',')[0], event.split(',')[1]) for event
                          in response.split('\n')[1:]])
    ...
    def test_csv_custom_token_notstream(self):
        ...
        self.assertIsNotNone(response)
        self.assertEqual([('2022-08-04 10:57:00.000', '18'),
                          ('2022-08-04 10:58:00.000', '12')],
                         [(event.split(',')[0], event.split(',')[1]) for event
                          in response.split('\n')[1:]])

    def test_csv_default_token_stream(self):
        result = iter([b'eventdate,count', b'', b'', b'', b'', b'', b'',
                       b'2022-08-04 12:38:00.000,6', b'', b'', b'',
    ...
    def test_csv_empty_event_token_stream(self):
        ...
        self.assertIsNotNone(response)
        self.assertEqual(
            [('eventdate', 'count'), ('2022-08-04 12:41:00.000', '4'),
             ('2022-08-04 12:42:00.000', '14')],
            [(event.split(',')[0], event.split(',')[1]) for event in response])

    def test_csv_custom_token_stream(self):
        result = iter([b'eventdate,count',
                       b'TOKENTOKENTOKENTOKENTOKENTOKEN'
                       b'2022-08-04 12:44:00.000,4',
        ...
        self.assertIsNotNone(response)
        self.assertEqual(
            [('eventdate', 'count'), ('2022-08-04 12:44:00.000', '4'),
    ...
    def test_tsv_default_token_notstream(self):
        ...
        self.assertIsNotNone(response)
        self.assertEqual(
            [('eventdate', 'count'), ('2022-08-04 11:10:00.000', '4'),
             ('2022-08-04 11:11:00.000', '169')],
            [(event.split('\t')[0], event.split('\t')[1]) for event in
             response.split('\n')])
    ...
    def test_tsv_empty_event_token_notstream(self):
        ...
        self.assertIsNotNone(response)
        self.assertEqual(
            [('eventdate', 'count'), ('2022-08-04 11:06:00.000', '24'),
             ('2022-08-04 11:07:00.000', '86')],
            [(event.split('\t')[0], event.split('\t')[1]) for event in
             response.split('\n')])
    ...
    def test_tsv_custom_token_notstream(self):
        ...
        self.assertIsNotNone(response)
        self.assertEqual(
            [('eventdate', 'count'), ('2022-08-04 11:02:00.000', '8'),
             ('2022-08-04 11:03:00.000', '22')],
            [(event.split('\t')[0], event.split('\t')[1]) for event in
             response.split('\n')])
    ...
    def test_tsv_default_token_stream(self):
        result = iter([b'eventdate\tcount', b'', b'', b'', b'', b'', b'',
                       b'2022-08-04 12:48:00.000\t4', b'', b'', b'',
        ...
        self.assertIsNotNone(response)
        self.assertEqual(
            [('eventdate', 'count'), ('2022-08-04 12:48:00.000', '4'),
    ...
    def test_tsv_empty_event_token_stream(self):
        result = iter(
            [b'eventdate\tcount', b'\t', b'\t', b'\t', b'\t', b'\t', b'\t',
             b'\t', b'\t', b'\t',
             b'2022-08-04 12:51:00.000\t12', b'\t', b'\t',
        ...
        self.assertIsNotNone(response)
        self.assertEqual(
            [('eventdate', 'count'), ('2022-08-04 12:51:00.000', '12'),
    ...
    def test_tsv_custom_token_stream(self):
        result = iter([b'eventdate\tcount',
                       b'TOKENTOKENTOKENTOKENTOKENTOKENTOKEN'
                       b'2022-08-04 12:54:00.000\t8',
        ...
        self.assertIsNotNone(response)
        self.assertEqual(
            [('eventdate', 'count'), ('2022-08-04 12:54:00.000', '8'),
