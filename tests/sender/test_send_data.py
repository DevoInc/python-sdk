...
class TestSender(unittest.TestCase):
    ...
    def test_compose_mem(self):
        self.assertEqual(
            Sender.compose_mem("test.tag"),
            "<14>Jan  1 00:00:00 %s test.tag: " % self.localhost,
        )

        self.assertEqual(
            Sender.compose_mem("test.tag", hostname="my-pc"),
        ...
        self.assertEqual(
            Sender.compose_mem(b"test.tag", date=b"1991-02-20 12:00:00", bytes=True),
    ...
    def test_tcp_rt_send(self):
        ...
        """
        try:
            engine_config = SenderConfigTCP(address=(self.tcp_server, self.tcp_port))
    ...
    def test_ssl_zip_send(self):
        ...
        try:
            ...
            for i in range(self.default_numbers_sendings):
                con.send(tag=self.my_bapp, msg=self.test_msg.encode("utf-8"), zip=True)
                con.flush_buffer()
                data_received = TestSender.read(con, 5000)
                print(b"\n" + data_received)
                if len(data_received) == 0:
    ...
    def test_fake_get_peer_cert_chain(self):
        ...
        fake_chain_cert = engine_config.fake_get_peer_cert_chain(self.chain)
    ...
    def test_open_file(self):
        ...
        with self.assertRaises(DevoSenderException):
